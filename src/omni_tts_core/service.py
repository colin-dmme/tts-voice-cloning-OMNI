...
from omni_tts_core.progress import ProgressCallback, ProgressEvent, check_cancel, emit_progress
...
class TtsService:
    ...
    def generate_audio(
        ...
        for index, chunk in enumerate(chunks, start=1):
            ...
            emit_progress(
                progress_callback,
                f"Đang tạo đoạn {index}/{len(chunks)}...",
                index - 1,
                len(chunks),
            )
            ...
            emit_progress(
                progress_callback,
                f"Hoàn tất đoạn {index}/{len(chunks)}.",
                index,
                len(chunks),
            )
        ...
        combined = concatenate_segments(audio_segments, sample_rate, request.sentence_pause_ms)
    ...
    def _generate_split_units(
        ...
        for unit_index, unit in enumerate(units, start=1):
            ...
            emit_progress(
                progress_callback,
                f"Đang tạo file {unit_index}/{len(units)}...",
                unit_index - 1,
                len(units),
            )
            ...
            child_request = request.model_copy(
                update={
                    ...
                    "output_stem": unit_stem,
                    "output_mode": "merged",
                }
            )
            result = self.generate_audio(
                child_request,
                _scale_progress(progress_callback, unit_index, len(units)),
                cancel_event,
            )
            audio_paths.append(result.audio_path)
            if result.srt_path is not None:
                srt_paths.append(result.srt_path)
            total_duration += result.duration_seconds
            total_segments += result.segment_count
            emit_progress(
                progress_callback,
                f"Hoàn tất file {unit_index}/{len(units)}.",
                unit_index,
                len(units),
            )
...
def _scale_progress(
    ...
    def scaled(event: ProgressEvent) -> None:
        unit_progress = event.current / event.total if event.total > 0 else 0.0
        current = (unit_index - 1) + unit_progress
        callback(
            ProgressEvent(
                message=f"File {unit_index}/{total_units}: {event.message}",
                current=current,
                total=total_units,
            )
        )
